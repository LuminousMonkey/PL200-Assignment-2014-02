...
// Node counter, so unique ids can be generated for GraphViz output.
 ...
 * startToken - First token in the rule.
 * endToken - Last token in the rule.
 * loc - Updated location so correct information can be passed back up
 *       the parse tree.
...
void outputNodeHeader(const char* const type,
                      ...
                      const YYLTYPE* const startToken,
                      const YYLTYPE* const endToken,
                      YYLTYPE *loc,
  ...
  if (graphVizOutput) {
    ...
    printf("Line: %d to %d. Non-terminal: %s\n", startToken->first_line,
            endToken->last_line, label);
  }

  loc->first_line = startToken->first_line;
  loc->last_line = endToken->last_line;
}

/*
 * outputGvNodeEdge
 *
 * Only does anything if GraphViz output is enabled.
 *
 ...
 * will be in the header file generated by Bison.
 */
...
basic_program:  declaration_unit {
                outputNodeHeader("basic_program", "Basic Program",
                                   &@1, &@1, &@$, &$$, &nodeCount);
...
declaration_unit:
                ...
                outputNodeHeader("declaration", "Declaration Unit",
                                   &@1, &@6, &@$, &$$, &nodeCount);
                outputGvNodeEdge(&$$, 6, &$3, &$4, &$5, &$6, &$7, &$8);
                };
...
opt_procedure_interface:
                ...
                outputNodeHeader("procedure_interface", "Opt Procedure Interface",
                                   &@1, &@3, &@$, &$$, &nodeCount);
...
opt_function_interface:
                ...
                outputNodeHeader("function_interface", "Opt Function Interface",
                                   &@1, &@3, &@$, &$$, &nodeCount);
                outputGvNodeEdge(&$$, 2, &$2, &$3); }
        |       { initNode(&$$); };
...
int main(int argc, char* argv[]) {
  // Check if GraphViz output is wanted.
  if (argc == 2) {
